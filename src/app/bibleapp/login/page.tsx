...
function LoginForm() {
  ...
  const redirect = searchParams.get("redirect") ?? "/bibleapp/dashboard";
  ...
  const handleGoogleLogin = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/bibleapp/api/auth/callback?next=${redirect}`,
      },
    });
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md space-y-6">
          ...
          <p className="text-center text-sm text-muted-foreground">
            ...
            <Link href="/bibleapp/register" className="text-primary font-medium hover:underline">
              Create one free
            </Link>
          </p>
        </div>
