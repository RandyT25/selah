...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterForm) => {
    ...
    const { data: signUpData, error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        emailRedirectTo: `${window.location.origin}/bibleapp/api/auth/callback?next=/bibleapp/dashboard`,
      },
    });
    ...
    if (signUpData.session) {
      router.push("/bibleapp/dashboard");
      router.refresh();
    } else {
      setRegistered(true);
    }
  ...
  const handleGoogleSignup = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/bibleapp/api/auth/callback`,
      },
    });
      ...
      <div className="min-h-screen flex items-center justify-center px-6">
        <div className="max-w-md w-full text-center space-y-4">
          ...
          <Button variant="outline" asChild>
            <Link href="/bibleapp/login">Return to Sign In</Link>
          </Button>
        </div>
      </div>
    );
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md space-y-6">
          ...
          <p className="text-center text-sm text-muted-foreground">
            ...
            <Link href="/bibleapp/login" className="text-primary font-medium hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
